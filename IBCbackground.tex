...
\WB{reviewer says: Section 2.1 I think could use a little extra references to some of the statements; the crypto references are top notch but a Silverman here or there wouldn't be bad for the interested reader; same holds for section 2.2}
\SG{DONE!}
...
\LDF{This sentence doesn't match quite well the ordinary / CSIDH case.}
%
...
%
In the supersingular case, the graph is Ramanujan, meaning it has optimal expansion properties (see Section 4 of~\cite{CGL}). The expansion properties imply that any two curves are connected by a path of length  $O(\log p)$, and that random walks on the graph quickly converge to the uniform distribution. \LDF{The stationary distribution is actually slightly different from uniform, except when $p=1\mod 12$. I'd write "stationary" instead of "uniform", but maybe do you think the word "stationary" is too mysterious?}
\SG{Probably better to say ``uniform apart from at most 2 special vertices''}
%
...
\paragraph{Two-party construction of an SIDH square.}
\LDF{Shall we completely remove this?}
...
%This key exchange scheme is secure against passive adversaries (i.e.\ adversaries who observe the messages between Alice and Bob but cannot change them) if it is difficult to recover $A$ from the knowledge of $(E/A, \phi_A(P_B), \phi_A(Q_B))$.
%This problem was called CSSI in~\cite{JDF11}, it is equivalent to finding a witness for a statement in $\R[SIDH]$ defined previously.

%\begin{definition}[Computational Supersingular Isogeny Problem (CSSI)]\label{def:cssi_prob} For fixed SIDH prime $p$, coprime integers $d,D$, base curve $E$, and $D$-torsion basis $P_B, Q_B \in E$, let $\phi_A : E \to E/A$ be an isogeny of degree $d$.
%  Given an SIDH public key $(E/A, \phi_A(P_B), \phi_A(Q_B))$, find an isogeny $\chi : E \to E/A$ of degree $d$ mapping $(P_B,Q_B)$ to $(\phi_A(P_B), \phi_A(Q_B))$.
% \end{definition}
...
\SG{I think typsetting looks clearer if we use $\fl_i = (\ell_i, 1 + \sqrt{-p})$ .}
